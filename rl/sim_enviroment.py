...
class SimulatedCustomEnv(Env):
    ...
    def step(self, actions):
        ...
        self.set_current_state(self.observation_space.predict(n=1, series=self.current_state, past_covariates=self.cov, verbose=self.verbose),state_before, cov_before)
    ...
    def reset(self):
        m = randint(0, len(self.series) - 7)
        # import pdb; pdb.set_trace()
        index = self.series.time_index
        self._reward = 0
        self.period = 7
        self.done = False
        self.current_state = self.series.slice(index[m], index[m + self.n_past - 1])
        self.cov = self.covariates.slice(index[m], index[m + self.n_past - 1])
